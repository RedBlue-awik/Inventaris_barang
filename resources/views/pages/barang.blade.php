                ...
                @foreach ($barangs as $barang)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right">
                            <button data-bs-toggle="modal" data-bs-target="#detailModal{{ $barang->id }}"
                    ...
                    <div class="modal fade" id="detailModal{{ $barang->id }}" tabindex="-1"
                        aria-labelledby="detailModalLabel{{ $barang->id }}" aria-hidden="true">
                            ...
                            <div class="modal-content rounded-xl border-0 shadow-lg">
                                <div class="modal-header border-b border-slate-100 p-4">
                                    <h5 class="modal-title font-bold text-slate-800"
                                        id="detailModalLabel{{ $barang->id }}">Detail Barang</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                        aria-label="Close"></button>
                                </div>
                                <div class="modal-body p-4">
                                    <div class="mb-4">
                                        <label class="block text-xs font-semibold text-slate-500 uppercase mb-1">Kode
                                            Barang</label>
                                        <p class="text-slate-800 font-medium">{{ $barang->kode_barang }}</p>
                                    </div>
                                    <div class="mb-4">
                                        <label class="block text-xs font-semibold text-slate-500 uppercase mb-1">Nama
                                            Barang</label>
                                        <p class="text-slate-800 font-medium">{{ $barang->nama_barang }}</p>
                                    </div>
                                    <div class="mb-4">
                                        <label
                                            class="block text-xs font-semibold text-slate-500 uppercase mb-1">Kategori</label>
                                        <p class="text-slate-800 font-medium">{{ $barang->kategori->nama_kategori ?? '-' }}
                                        </p>
                                    </div>
                                    <div class="mb-4">
                                        <label
                                            class="block text-xs font-semibold text-slate-500 uppercase mb-1">Supplier</label>
                                        <p class="text-slate-800 font-medium">{{ $barang->supplier->nama_supplier ?? '-' }}
                                        </p>
                                    </div>
                                    <div class="mb-4">
                                        <label
                                            class="block text-xs font-semibold text-slate-500 uppercase mb-1">Satuan</label>
                                        <p class="text-slate-800 font-medium">{{ $barang->satuan ?? '-' }}</p>
                                    </div>
                                    <div class="mb-4">
                                        <label class="block text-xs font-semibold text-slate-500 uppercase mb-1">Stok Saat
                                            Ini</label>
                                        <p class="text-slate-800 font-bold text-lg">{{ $barang->stok_saat_ini }}
                                            {{ $barang->satuan }}</p>
                                    </div>
                                    <div class="mb-4">
                                        <label class="block text-xs font-semibold text-slate-500 uppercase mb-1">Stok
                                            Minimum</label>
                                        <p class="text-slate-800 font-medium">{{ $barang->stok_minimum }}
                                            {{ $barang->satuan }}</p>
                                    </div>
                                    <div class="mb-4">
                                        <label class="block text-xs font-semibold text-slate-500 uppercase mb-1">Lokasi
                                            Rak</label>
                                        <p class="text-slate-800 font-medium">{{ $barang->lokasi_rak ?? '-' }}</p>
                                    </div>
                                    <div class="mb-4">
                                        <label
                                            class="block text-xs font-semibold text-slate-500 uppercase mb-1">Kondisi</label>
                                        @php
                                            $statusClass = '';
                                            switch ($barang->kondisi) {
                                                case 'baik':
                                        ...
                                        @endphp
                                        <span
                                            class="px-3 py-1 rounded-lg text-xs font-bold {{ $statusClass }}">{{ ucfirst($barang->kondisi) }}</span>
                                    </div>
                                    <div class="mb-4">
                                        <label class="block text-xs font-semibold text-slate-500 uppercase mb-1">Tanggal
                                            Ditambahkan</label>
                                        <p class="text-slate-800">{{ $barang->created_at->format('d M Y H:i') }}</p>
                                    </div>
                                </div>
                                        ...
                                        <div class="modal-body row p-4">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="nama_barang"
                                                    class="form-control rounded-lg w-full" placeholder="Contoh: Pensil"
                                                    required value="{{ $barang->nama_barang }}">
                                            </div>
                                            <div class="mb-3">
                                                <label
                                                    class="block text-sm font-medium text-slate-700 mb-1">Kategori</label>
                                                <select name="kategori_id" class="form-control rounded-lg w-full"
                                                    required>
                                            ...
                                            <div class="mb-3">
                                                <label
                                                    class="block text-sm font-medium text-slate-700 mb-1">Supplier</label>
                                                <select name="supplier_id" class="form-control rounded-lg w-full"
                                                    required>
                                                    <option value="" hidden disabled selected>Pilih Supplier</option>
                                                    @foreach ($suppliers as $sup)
                                                        <option value="{{ $sup->id }}"
                                                            {{ $barang->supplier_id == $sup->id ? 'selected' : '' }}>
                                                            {{ $sup->nama_supplier }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="col-md-4 mb-3">
                                                <label class="block text-sm font-medium text-slate-700 mb-1">Stok
                                                    Awal</label>
                                                <input type="number" name="stok_saat_ini"
                                                    class="form-control rounded-lg w-full"
                                                    value="{{ $barang->stok_saat_ini }}" required>
                                            </div>
                                            <div class="col-md-4 mb-3">
                                                <label class="block text-sm font-medium text-slate-700 mb-1">Stok
                                                    Min</label>
                                            ...
                                            <div class="mb-3">
                                                <label class="block text-sm font-medium text-slate-700 mb-1">Lokasi
                                                    Rak</label>
                                            ...
                                            <div class="mb-3">
                                                <label
                                                    class="block text-sm font-medium text-slate-700 mb-1">Kondisi</label>
    ...
    <script>
        $(document).ready(function() {
            $('#table-barang').DataTable({
                ...
                dom: '<"flex flex-col gap-4 pb-4 md:flex-row md:justify-between md:items-center"lf>rt<"flex flex-col gap-4 md:flex-row md:justify-between md:items-center"ip>',
                language: {
                    search: "_INPUT_",
