...
@section('header_title', 'Data Permintaan')
            ...
            <tbody>
                @foreach ($permintaans as $item)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-slate-600">{{ $item->barang->nama_barang ?? 'Unknown' }}
                            ({{ $item->jumlah_diminta }})</td>
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content rounded-xl border-0 shadow-lg">
                                ...
                                <div class="modal-body p-4">
                                    <div class="mb-4">
                                        <label class="block text-xs font-semibold text-slate-500 uppercase mb-1">No.
                                            Permintaan</label>
                                        <p class="text-slate-800 font-medium">{{ $item->no_permintaan }}</p>
                                    </div>
                                    <div class="mb-4">
                                        <label
                                            class="block text-xs font-semibold text-slate-500 uppercase mb-1">Pemohon</label>
                                        <p class="text-slate-800 font-medium">{{ $item->permohonan->name ?? 'Unknown' }}
                                        </p>
                                    </div>
                                    <div class="mb-4">
                                        <label
                                            class="block text-xs font-semibold text-slate-500 uppercase mb-1">Barang</label>
                                        <p class="text-slate-800 font-medium">{{ $item->barang->nama_barang ?? 'Unknown' }}
                                        </p>
                                    </div>
                                    <div class="mb-4">
                                        <label class="block text-xs font-semibold text-slate-500 uppercase mb-1">Jumlah
                                            Diminta</label>
                                        <p class="text-slate-800 font-medium">{{ $item->jumlah_diminta }}
                                            {{ $item->barang->satuan ?? '' }}</p>
                                    </div>
                                    <div class="mb-4">
                                        <label class="block text-xs font-semibold text-slate-500 uppercase mb-1">Jumlah
                                            Disetujui</label>
                                        <p class="text-slate-800 font-medium">{{ $item->jumlah_disetujui ?? '-' }}
                                            {{ $item->barang->satuan ?? '' }}</p>
                                    </div>
                                    <div class="mb-4">
                                        <label
                                            class="block text-xs font-semibold text-slate-500 uppercase mb-1">Keperluan</label>
                                        <p class="text-slate-800">{{ $item->keperluan ?? '-' }}</p>
                                    </div>
                                    <div class="mb-4">
                                        <label
                                            class="block text-xs font-semibold text-slate-500 uppercase mb-1">Status</label>
                                        @php
                                            $statusColors = [
                                                'menunggu' => 'bg-yellow-100 text-yellow-800 border-yellow-200',
                                                'disetujui' => 'bg-green-100 text-green-800 border-green-200',
                                        ...
                                        @endphp
                                        <span
                                            class="px-3 py-1 rounded-full text-xs font-semibold border {{ $color }}">
                                            {{ ucfirst($item->status) }}
                                        </span>
                                    </div>
                                    <div class="mb-4">
                                        <label class="block text-xs font-semibold text-slate-500 uppercase mb-1">Disetujui
                                            Oleh</label>
                    ...
                    @if (
                        $item->status === 'menunggu' &&
                            ((Auth::user()->role === 'staff' && Auth::user()->id === $item->permohonan_id) ||
                                ...
                                <div class="modal-content rounded-xl border-0 shadow-lg">
                                    <form action="{{ route('permintaan.update', $item->id) }}" method="POST">
                                        ...
                                        <div class="modal-body p-4">
                                            ...
                                            <div class="mb-3">
                                                <label class="block text-sm font-medium text-slate-700 mb-1">Jumlah
                                                    Diminta</label>
                                                <input type="number" name="jumlah_diminta"
                                                    class="form-control rounded-lg w-full"
                                                    value="{{ $item->jumlah_diminta }}" min="1" required>
                                            </div>
                                            <div class="mb-3">
                                                <label
                                                    class="block text-sm font-medium text-slate-700 mb-1">Keperluan</label>
                                        ...
                                        <div class="modal-body p-4">
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <select name="status" id="statusSelect{{ $item->id }}"
                                                    ...
                                                    onchange="toggleJumlahField({{ $item->id }})">
                                            ...
                                            <div id="jumlahField{{ $item->id }}" class="mb-4"
                                                style="display: none;">
                                                <label class="block text-sm font-medium text-slate-700 mb-1">Jumlah
                                                    Disetujui</label>
                                                <input type="number" name="jumlah_disetujui"
                                                    id="jumlahInput{{ $item->id }}"
                                                    class="form-control rounded-lg w-full" min="1"
                                                    max="{{ $item->jumlah_diminta }}"
                                                    placeholder="Masukkan jumlah yang disetujui">
                                                <small class="text-slate-500">Maksimal: {{ $item->jumlah_diminta }}
                                                    {{ $item->barang->satuan }}</small>
                                            </div>
                                        </div>
                                        <div class="modal-footer border-t border-slate-100 p-4">
                                            <button type="button"
                                                ...
                                                data-bs-dismiss="modal">Batal</button>
                                            <button type="submit"
                                                class="px-4 py-2 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-700 transition">Proses
                                                Permintaan</button>
                                        </div>
                                    </form>
                                </div>
                    ...
                    <div class="modal fade" id="modalHapus{{ $item->id }}" tabindex="-1">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content rounded-xl border-0 shadow-lg">
                                ...
                                <div class="modal-body p-4">
                                    <p>Apakah Anda yakin ingin menghapus permintaan
                                        <strong>{{ $item->no_permintaan }}</strong>?
                                    </p>
